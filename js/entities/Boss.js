...
export class Boss extends Phaser.Physics.Arcade.Sprite {
  constructor(scene, x, y, def, hp, dmg) {
    ...
    this.def = def;
    // Animated boss sprite (imported enemy sheets) — play its looping frames.
    if (def.anim && scene.anims.exists(def.anim + '-move')) this.play(def.anim + '-move', true);
    // Boss balance: extra HP (resistance) and slightly higher damage.
    const B = CONFIG.balance;
    this.maxHP = Math.round(hp * (B ? B.bossHpMult : 1)); this.hp = this.maxHP;
    this.contactDmg = Math.round(dmg * (B ? B.bossDmgMult : 1));
  ...
  die() {
    ...
    if (this.anims && this.anims.isPlaying) this.anims.stop();
