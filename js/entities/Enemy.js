...
export class Enemy extends Phaser.Physics.Arcade.Sprite {
  ...
  applyType(typeKey, x, y, { elite = false } = {}) {
    ...
    this.maxHP = def.hp * hpMult * (em ? em.hpMult : 1) * (CONFIG.balance ? CONFIG.balance.enemyHpMult : 1);
    this.hp = this.maxHP;
    ...
    // Frame-by-frame animation for imported enemy sheets (real frames only).
    if (def.anim && this.scene.anims.exists(def.anim + '-move')) {
      this.setOrigin(0.5, 0.92);
      this.play(def.anim + '-move', true);
  ...
  die() {
    ...
    // Freeze on the current frame during the death squash (avoid loop flicker).
