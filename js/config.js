...
export const CONFIG = {
  ...
  weapon: {
    damage: 8,
    cooldownMs: 720,       // time between shots (attack speed) — slower, rewards positioning
    range: 440,            // max target-acquire distance
    critChance: 0.06,      // 0..1
    critMult: 1.8,         // crit damage multiplier
    projSpeed: 460,
  ...
  enemies: {
    types: {
      ...
      // ===== FINAL MAP — Realm of Hoodlust (biome-exclusive) =====
      doom_knight: { key: 'doom_knight', hp: 320, speed: 74, dmg: 40, xp: 20, scale: 1.15, radius: 13, weight: 1, behavior: 'chase' },
      void_wraith: { key: 'void_wraith', hp: 240, speed: 92, dmg: 34, xp: 18, scale: 1.05, radius: 12, weight: 1, behavior: 'teleport', tint: 0xc86aff, blinkCd: 2.6, blinkRange: 240 },
      hell_hound:  { key: 'hell_hound',  hp: 200, speed: 230, dmg: 30, xp: 17, scale: 1.0, radius: 11, weight: 1, behavior: 'chase', tint: 0xff5a3a },
    ...
    // ===== IMPORTED ENEMY SHEETS — one enemy per map (Maps 1-8) =====
    // Real frame-by-frame animated sprites (see js/data/enemyAnims.js). Each is
    // exclusive to its assigned region and has a distinct AI behaviour.
    imported: {
      // Map 1 — slow, easy, short range.
      enemy1: { key: 'enemy1', anim: 'enemy1', hp: 30, speed: 52, dmg: 6, xp: 3, scale: 0.34, radius: 16, behavior: 'chase', region: 'magic_forest' },
      // Map 2 — faster, more aggressive.
      enemy2: { key: 'enemy2', anim: 'enemy2', hp: 55, speed: 96, dmg: 10, xp: 4, scale: 0.30, radius: 16, behavior: 'chase', region: 'enchanted_garden' },
      // Map 3 — dodges (teleports) sometimes.
      enemy3: { key: 'enemy3', anim: 'enemy3', hp: 90, speed: 60, dmg: 14, xp: 6, scale: 0.20, radius: 22, behavior: 'teleport', blinkCd: 3.4, blinkRange: 200, region: 'moonlight_graveyard' },
      // Map 4 — long attack range.
      enemy4: { key: 'enemy4', anim: 'enemy4', hp: 90, speed: 58, dmg: 12, xp: 6, scale: 0.30, radius: 16, behavior: 'ranged', shootCd: 1.6, keepDist: 260, bulletSpd: 260, bulletDmg: 14, region: 'crystal_valley' },
      // Map 5 — area attacks (explosive burst).
      enemy5: { key: 'enemy5', anim: 'enemy5', hp: 130, speed: 66, dmg: 16, xp: 8, scale: 0.32, radius: 18, behavior: 'exploder', explodeR: 120, explodeDmg: 30, region: 'ancient_ruins' },
      // Map 6 — dash attacks (fast blink-rush).
      enemy6: { key: 'enemy6', anim: 'enemy6', hp: 150, speed: 150, dmg: 20, xp: 9, scale: 0.32, radius: 17, behavior: 'teleport', blinkCd: 2.4, blinkRange: 260, region: 'corrupted_jungle' },
      // Map 7 — elite, high damage.
      enemy7: { key: 'enemy7', anim: 'enemy7', hp: 260, speed: 84, dmg: 30, xp: 14, scale: 0.34, radius: 18, behavior: 'chase', region: 'mushroom_forest' },
      // Map 8 — mini-boss: large radius, summons, tanky.
      enemy8: { key: 'enemy8', anim: 'enemy8', hp: 520, speed: 58, dmg: 30, xp: 22, scale: 0.42, radius: 22, behavior: 'summoner', summonCd: 5, summonKey: 'enemy8_add', summonN: 2, region: 'frozen_wasteland' },
      // Mini-boss adds (small, share the mini-boss sprite).
      enemy8_add: { key: 'enemy8', anim: 'enemy8', hp: 60, speed: 92, dmg: 12, xp: 4, scale: 0.24, radius: 12, behavior: 'chase', region: 'frozen_wasteland' },
    },
  ...
  difficulty: {
    ...
    perLevel: 0.35,        // index gained per in-run level (20 levels = one map)
  ...
  // PHASE: balance pass — reward skill over button-spam. Player output is
  // dialed back and enemies/bosses are sturdier (smooth, not unfair).
  balance: {
    playerDmgMult: 0.8,    // -20% player damage
    playerCritChanceMult: 0.75,
    playerHealMult: 0.85,  // healing/regen a touch weaker
    enemyHpMult: 1.25,     // +25% normal-enemy HP
    bossHpMult: 1.35,      // bosses are tankier (resistance)
    bossDmgMult: 1.1,
  },
...
// Merge the imported enemy-sheet types into the main lookup so Enemy/Spawner
// resolve them exactly like any other type.
Object.assign(CONFIG.enemies.types, CONFIG.enemies.imported);
