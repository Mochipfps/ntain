...
import { LEVELS_PER_MAP } from '../systems/WorldState.js';
...
export class GameScene extends Phaser.Scene {
  ...
  init(data) {
    ...
    // Carry live progression across transitions (read in create()).
    this._carryData = data && data.carry;
  ...
  create() {
    ...
    // Carry live progression across map/dungeon transitions so changing areas
    // never resets XP, level or coins (temporary combat state is not carried).
    const carry = this._carryData;
    if (carry) {
      this.progression.level = carry.level || 1;
      this.progression.xp = carry.xp || 0;
      this.progression.coins = carry.coins || 0;
      // Recompute the XP-to-next threshold for the restored level.
      let need = CONFIG.level.baseXP;
      for (let i = 1; i < this.progression.level; i++) need = Math.round(need * CONFIG.level.growth);
      this.progression.xpToNext = need;
    }
    ...
    this.ui.hud.show();
    // Region name + world level on the HUD (progression map shows its 20-level span).
    const rname = REGIONS[this.regionId] ? REGIONS[this.regionId].name : map.name;
    const mapIdx = this.world ? this.world.mapIndex(this.regionId) : -1;
    this.ui.hud.setRegion(mapIdx >= 0 ? `${rname} (Area ${mapIdx + 1})` : rname, this.world ? this.world.worldLevel : 1);
  ...
  dealHit(target, dmg, crit, fx, fy) {
    ...
    if (ls > 0) this.playerHP = Math.min(this.progression.stats.maxHP, this.playerHP + dmg * ls * (CONFIG.balance ? CONFIG.balance.playerHealMult : 1));
  ...
  _buildPortals() {
    ...
    links.forEach((link, i) => {
      // PRODUCTION: a portal to a locked region must not exist at all —
      // no sprite, no collision, no label, no interaction. It appears only
      // once the destination is unlocked (previous region's boss cleared).
      const townDest = REGIONS[link.to] && REGIONS[link.to].town;
      if (this.world && !townDest && !this.world.isUnlocked(link.to)) return;
      ...
      // Floating label (region name — or ??? if not yet discovered).
  ...
  _enterDungeon(player, gate) {
    ...
    this.cameras.main.once('camerafadeoutcomplete', () => {
      this.scene.restart({ joystick: this.joystick, character: this.charDef.id, map: this.mapDef.id, region: this.regionId, dungeon: true, carry: this._carry() });
    });
  }
  ...
  _exitDungeon() {
    ...
    this.cameras.main.fadeOut(400, 6, 4, 12);
    this.cameras.main.once('camerafadeoutcomplete', () => {
      this.scene.restart({ joystick: this.joystick, character: this.charDef.id, map: this.mapDef.id, region: this.regionId, carry: this._carry() });
    });
  }
  ...
  _enterPortal(player, gate) {
    ...
    this.cameras.main.once('camerafadeoutcomplete', () => {
      this.scene.restart({ joystick: this.joystick, character: this.charDef.id, map: link.region.map, region: link.to, carry: this._carry() });
    });
  }
  ...
  /** Snapshot the live run so map/dungeon changes never lose progress. */
  /** Snapshot the live run so map/dungeon changes never lose progress. */
  _carry() {
    return { level: this.progression.level, xp: this.progression.xp, coins: this.progression.coins };
  }

  _saveSession() {
    if (this.over || this.isTown) return;
    this.save.saveSession({
      map: this.mapDef.id, region: this.regionId, character: this.charDef.id,
      level: this.progression.level, xp: this.progression.xp, coins: this.progression.coins,
      weapons: this.weaponMgr ? this.weaponMgr.owned().map(w => w.id) : [],
    });
  }
  _gainXP(v) {
  ...
  _openLevelUp() {
    ...
    this.ui.dialogs.showLevelUp(choices, this.progression.level, (u) => {
      ...
      this.syncStats();
      this.pendingLevels -= 1;
      // Map completion: reaching the map's final level (20) clears the area
      // and advances to the next map. Not in towns/dungeons.
      if (this._checkAreaClear()) return;
      if (this.pendingLevels > 0) this._openLevelUp();
      else this._resumeFromDialog();
    });
  }

  /** True if this level-up completed the map (level 20) and area-clear fired. */
  _checkAreaClear() {
    if (this._areaClearing || this.isTown || this.isDungeon || !this.world) return false;
    if (this.world.mapIndex(this.regionId) < 0) return false;      // not a progression map
    if (this.progression.level < LEVELS_PER_MAP) return false;
    this._areaClearing = true;
    this._areaCleared();
    return true;
  }

  _areaCleared() {
    // Persist progress, unlock + queue the next map.
    this.save.data.currentMap = this.regionId;
    this.save.data.currentLevel = this.progression.level;
    this.save.save();
    const next = this.world.completeMap(this.regionId);
    this.world.recompute(this.save.data.highestLevel || this.progression.level);

    // Victory banner: "Area Cleared".
    const { width, height } = this.scale;
    const overlay = this.add.rectangle(0, 0, width, height, 0x0a0a12, 0.72)
      .setOrigin(0, 0).setScrollFactor(0).setDepth(DEPTH.HUD + 10);
    const title = this.add.text(width / 2, height / 2 - 30, 'AREA CLEARED', {
      fontFamily: '"Press Start 2P", monospace', fontSize: '30px', color: '#f2d16b',
      stroke: '#2a1500', strokeThickness: 6, align: 'center',
    }).setOrigin(0.5).setScrollFactor(0).setDepth(DEPTH.HUD + 11).setScale(0.6).setAlpha(0);
    const sub = this.add.text(width / 2, height / 2 + 24,
      next ? 'Loading next area…' : 'All areas cleared — champion!', {
      fontFamily: 'monospace', fontSize: '15px', color: '#cfe0ff',
      stroke: '#000', strokeThickness: 3, align: 'center',
    }).setOrigin(0.5).setScrollFactor(0).setDepth(DEPTH.HUD + 11).setAlpha(0);
    ...
    this.time.delayedCall(2200, () => {
      if (next) {
        this.cameras.main.fadeOut(500, 8, 10, 16);
        this.cameras.main.once('camerafadeoutcomplete', () => {
          // Spawn at the start of the new map (fresh run, next region).
          this.scene.restart({ joystick: this.joystick, character: this.charDef.id, map: REGIONS[next].map, region: next, carry: this._carry() });
        });
      } else {
        // Final map done — end the run in triumph.
  ...
  _togglePause() {
    ...
    this.ui.dialogs.showPause(
      ...
      () => { this._saveSession(); this._resumeFromDialog(); this._quitToMenu(); }
    );
  }
  ...
  update(time, delta) {
    ...
    // Regen.
    if (st.regen > 0) { this._regenAcc += dt; if (this._regenAcc >= 1) { this._regenAcc -= 1; this.playerHP = Math.min(st.maxHP, this.playerHP + st.regen * (CONFIG.balance ? CONFIG.balance.playerHealMult : 1)); } }
