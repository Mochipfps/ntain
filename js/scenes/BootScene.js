...
import { ENEMY_SHEETS } from '../data/enemyAnims.js';
...
export class BootScene extends Phaser.Scene {
  ...
  preload() {
    ...
    ['floor_sky', 'floor_volcano', 'floor_frost', 'floor_jungle', 'floor_mush', 'floor_desert',
     ...
     'floor_hoodlust', 'statue', 'chain', 'torch', 'dark_crystal', 'bones', 'dead_tree',
     'lava_vent', 'ruined_tower', 'ancient_door', 'magic_circle', 'collapsed_wall']
      .forEach(k => this.load.image(k, 'assets/background/' + k + '.png'));
    ['sky_wisp', 'magma_imp', 'ice_shard', 'corrupt_spore', 'myco_crawler', 'sand_lurker',
     ...
     'doom_knight', 'void_wraith', 'hell_hound', 'boss_hoodlust']
      .forEach(k => this.load.image(k, 'assets/enemies/' + k + '.png'));
    // Imported enemy sheets (Maps 1-8) — spritesheets for frame-by-frame anim.
    for (const [id, m] of Object.entries(ENEMY_SHEETS))
      this.load.spritesheet(id, 'assets/enemies/' + id + '.png', { frameWidth: m.w, frameHeight: m.h });
  ...
  create() {
    ...
    // Register looping "move" animations for the 8 imported enemy sheets.
    for (const [id, m] of Object.entries(ENEMY_SHEETS)) {
      if (!this.textures.exists(id)) continue;
      this.anims.create({ key: id + '-move', frames: this.anims.generateFrameNumbers(id, { start: 0, end: m.count - 1 }), frameRate: m.fps, repeat: -1 });
    }
