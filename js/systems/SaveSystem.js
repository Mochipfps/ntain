...
export class SaveSystem {
  ...
  /** Persist a live in-run session so map/dungeon changes never lose progress.
      Only meta-progression + session snapshot; temporary combat state is not saved. */
  saveSession(sess) {
    this.data.session = sess ? {
      map: sess.map, region: sess.region, character: sess.character,
      level: sess.level, xp: sess.xp, coins: sess.coins,
      weapons: sess.weapons || [], ts: Date.now(),
    } : null;
    this.save();
  }
