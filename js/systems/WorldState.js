...
// Every map is one region and contains exactly this many levels. Reaching the
// final level of a map clears the area and unlocks the next map.
export const LEVELS_PER_MAP = 20;
...
export class WorldState {
  ...
  // ---- map / level structure (20 levels per map) ----
  /** Index of a region within the progression chain (0-based), or -1. */
  mapIndex(id) { return PROGRESSION_ORDER.indexOf(id); }
  /** The next region in the chain after `id`, or null at the end. */
  nextRegion(id) { const i = this.mapIndex(id); return (i >= 0 && i + 1 < PROGRESSION_ORDER.length) ? PROGRESSION_ORDER[i + 1] : null; }
  /** Global level band for a map, e.g. Map 2 → [21, 40]. */
  levelBand(id) { const i = Math.max(0, this.mapIndex(id)); return [i * LEVELS_PER_MAP + 1, (i + 1) * LEVELS_PER_MAP]; }

  /** Clear a map: mark it complete, unlock + set the next region current, save. */
  completeMap(id) {
    this.markBossCleared(id);
    const next = this.nextRegion(id);
    if (next) { this.current = next; this.discover(next); }
    this._persist();
    return next;
  }
