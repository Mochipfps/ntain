...
// Placeholder reward categories — actual rewards are NOT revealed yet. Each
// tier shows a mystery silhouette + category label; icons are blurred in the UI.
const PLACEHOLDER_FREE = [
  'New Skills', 'Exclusive Cosmetics', 'Seasonal Collectibles', 'Token Rewards',
  'New Powers', 'Special Event Rewards', 'Exclusive Outfits', 'Future Unlockables',
  'New Characters', 'NFT Rewards',
];
const PLACEHOLDER_ACCESS = [
  'NFT Rewards', 'Exclusive Outfits', 'New Characters', 'Token Rewards',
  'Special Event Rewards', 'New Powers', 'Seasonal Collectibles', 'Exclusive Cosmetics',
  'Future Unlockables', 'New Skills',
];

// Cosmetic reward tables (cosmetic only). f=free, p=access-pass.
function bpReward(tier) {
  ...
  return {
    free: f, premium: p,
    // hidden display categories (shown blurred)
    freeLabel: PLACEHOLDER_FREE[tier % 10], accessLabel: PLACEHOLDER_ACCESS[tier % 10],
  };
}
...
export class LiveOps {
  ...
  isPremium() { return !!this._accessPassVerified; }
  isAccessPass() { return !!this._accessPassVerified; }
  // Access Pass ownership is verified live on-chain (session only — never saved,
  // so it always reflects current NFT ownership).
  setAccessPass(owned) { this._accessPassVerified = !!owned; }
  addBattleXp(n) {
