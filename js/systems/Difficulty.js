...
export class Difficulty {
  ...
  update(dt) {
    this.elapsed += dt;
    // In-run level adds a smooth difficulty ramp (each of a map's 20 levels
    // makes enemies gradually stronger — no sudden spikes).
    const lvl = (this.scene.progression && this.scene.progression.level) || 1;
    const levelBonus = (lvl - 1) * (this.d.perLevel || 0.35);
    // Live index = region/world baseline + level ramp + time survived + boss kills.
