...
import { validateUsername, validateUrl, validateErc20 } from './validation.js';
...
async function task4_comment_and_wallet() {
  ...
  const link = await showPopup({
    ...
    input: { label: 'Comment URL', placeholder: 'https://x.com/...', validate: validateUrl },
    primary: { label: 'Submit' },
  });
  ...
  const wallet = await showPopup({
    ...
    input: {
      ...
      hint: 'Must be a valid ERC-20 address (0x + 40 hex characters).',
  ...
  async _finalize() {
    ...
    while (!ok) {
      showPopup({
        ...
        primary: { label: 'Please wait' },
      });
      const res = await submitWhitelist(entry);
      closeActive();
      if (res.ok) { ok = true; break; }
      // Beautiful failure popup — player can retry (data is kept).
      await showPopup({
