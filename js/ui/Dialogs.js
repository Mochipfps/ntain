...
export class Dialogs {
  constructor() {
    ...
    this.pause.querySelectorAll('[data-pause]').forEach((b) => {
      b.addEventListener('click', () => {
        ...
        if (a === 'controls') { document.getElementById('controls-window')?.classList.remove('hidden'); return; }
        this._hide(this.pause);
        if (a === 'resume') this._cb('onResume');
        if (a === 'quit')   this._cb('onQuit');
        if (a === 'exit')   this._cb('onExit');
      });
    });
    // Game over buttons.
  ...
  showPause(onResume, onQuit, onExit) {
    this.handlers.onResume = onResume;
    this.handlers.onQuit = onQuit;
    this.handlers.onExit = onExit;
    this._showBackdrop();
