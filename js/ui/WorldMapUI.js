...
export class WorldMapUI {
  ...
  _detail(id) {
    ...
    // Fast-travel only to regions that are both discovered AND unlocked by
    // sequential progression (previous region cleared).
    const travelable = r && this.world.isDiscovered(id) && this.world.isUnlocked(id);
    if (!travelable) {
      this.detailEl.innerHTML = (r && this.world.isDiscovered(id))
        ? 'Locked — clear the previous region to unlock this destination.'
        : 'Undiscovered region — reach it through a portal to unlock fast travel.';
  ...
  _travel() {
    ...
    if (!id || !this.world.isDiscovered(id) || !this.world.isUnlocked(id)) return;
    this.hide();
