  ...
  play() {
    ...
    const openSelection = () => {
      ...
      window.HOODLUST.ui.select.open(({ character, map }) => {
        this.el.desktop.classList.add('hidden');
        if (this.joystick) this.joystick.show();
        this.game.scene.start('GameScene', { joystick: this.joystick, character, map, region: map });
        this.started = true;
      });
    };

    // Production gate: only on-chain-verified HoodLust holders may play.
    const web3 = window.HOODLUST.ui.web3;
    if (web3.isVerified()) openSelection();
    else web3.openGate(() => openSelection());
  }

  /** PHASE 7.5 — travel to a (discovered) region via the World Map. Passes
      the same holder gate as Play, then starts/switches the run there. */
  travelTo(regionId) {
    if (!this.ready) return;
    const region = REGIONS[regionId];
    const map = region ? region.map : regionId;
    const character = localStorage.getItem('hoodlust-char') || 'lush_default';
    const go = () => {
      this.el.desktop.classList.add('hidden');
      if (this.joystick) this.joystick.show();
      this.game.scene.start('GameScene', { joystick: this.joystick, character, map, region: regionId });
      this.started = true;
    };
    const web3 = window.HOODLUST.ui.web3;
    if (web3.isVerified()) go();
    else web3.openGate(() => go());
  }
