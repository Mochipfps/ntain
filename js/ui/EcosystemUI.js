...
export class EcosystemUI {
  ...
  showBattlePass() {
    ...
    this._bpBindDrag();
    // Battle Pass access is verified on-chain, INDEPENDENTLY of game access,
    // against the Access Pass collection. Re-checks live each time it opens.
    this._verifyAccessPass();
  }

  async _verifyAccessPass() {
    const web3 = window.HOODLUST.ui.web3;
    const addr = web3 && web3.session && web3.session.wallet;
    // Not connected → keep locked (game access is separate; may be verified
    // via the game gate, but Access Pass needs its own wallet check).
    if (!addr) { this.liveops.setAccessPass(false); this._renderBp(); return; }
    const head = document.getElementById('bp-access');
    if (head) head.innerHTML = '<div class="bp-access-off"><span>Checking Access Pass ownership…</span></div>';
    try {
      const n = await web3.verify.accessPassBalance(addr);
      this.liveops.setAccessPass(n > 0);
    } catch (_) {
      this.liveops.setAccessPass(false);
    }
    this._renderBp();
  }
  _renderBp() {
    ...
    document.getElementById('bp-access').innerHTML = access
      ? `<div class="bp-access-on">&#10004; HoodLust Access Pass verified — Access Pass rewards unlocked.</div>`
      : `<div class="bp-access-off">
           <div class="bp-access-title">&#128274; Hoodlust Access Pass required.</div>
           <div class="bp-access-msg">Own the Hoodlust Access Pass to unlock Battle Pass progression and exclusive seasonal rewards.</div>
         </div>`;

    // Horizontal tier cards (Free + Access Pass reward per tier).
    document.getElementById('bp-track').innerHTML = lo.bpTable().map(r => {
      ...
      const cell = (track, label) => {
        ...
        const tag = track === 'premium' ? 'ACCESS PASS' : 'FREE';
        return `<div class="bp-reward ${cls}" data-tier="${r.tier}" data-track="${track}">
            ...
            ${claimed ? '<div class="bp-reward-state claimed">CLAIMED \u2714</div>'
              ...
              : locked ? '<div class="bp-reward-state locked">&#128274; Access Pass</div>'
